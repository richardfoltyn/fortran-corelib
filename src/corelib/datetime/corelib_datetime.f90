...
pure function timedelta_strftime (self, fmt) result(res)
    ...
    do while (i_at <= n)
        if (fmt(i_at:i_at) == '%') then
            ...
            select case (c)
            case ('%')
                call assert_size (j_at)
                buf(j_at:j_at) = '%'
                j_at = j_at + 1
            case ('d')
                fwidth = 1
                if (self%days > 0) fwidth = floor(log10(real(self%days)))
                call assert_size (j_at + fwidth - 1)
...
contains
    ...
    pure subroutine assert_size (i)
        ...
        if (len(buf) < i) then 
            newlen = i + BUFFER_INCR
            allocate (character (len=newlen) :: work)
        
            work(1:len(buf)) = buf
            call move_alloc (work, buf)
        end if
    end subroutine
